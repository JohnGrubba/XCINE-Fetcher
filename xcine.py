...
url = "https://xcine.me/serien-disney-phineas-und-ferb-8450-stream/folge-6"

r = requests.get(url)

episode_id = r.text[r.text.find(url, r.text.find('<a class="current"')):]
episode_id = int(episode_id[:episode_id.find(' title=')].replace('" data-name="" data-episode-id="', '').replace(url, "").replace('"', ''))
print(episode_id)
mov_id = r.text[r.text.find("var mov_id = "):]
mov_id = mov_id[:mov_id.find(";")]
mov_id = int(mov_id.replace("var mov_id = ", "").replace("'", ''))
print(mov_id)
